...
## 2026-06-22 외부 공고 RAG형 수집/등록 명령 구현 작업

- AI 채팅에서 `공고 수집`, `관심 사이트 공고`, `캘린더 일정 등록` 표현을 인식한다.
- `collect_sites` intent는 지정된 외부 사이트 메뉴의 공고를 수집해 후보 목록으로 저장한다.
- `collect_and_register_sites` intent는 수집 후보를 마감일 기준 캘린더 일정으로 등록/갱신한다.
- 현재 RAG 범위는 벡터 DB 기반 장기 지식 검색이 아니라, 지정 웹사이트 실시간 수집 결과를 구조화해 일정 도구 실행에 사용하는 검색 증강 실행 방식이다.
- 등록 결과는 채팅 응답에 수집 후보 수, 캘린더 반영 수, 대표 일정 목록으로 표시한다.
